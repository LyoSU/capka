...
> Уточнення при реалізації: у бульбашці переглядні типи (`previewKind !== null`)
> відкривають Quick Look, а непереглядні (напр. `.zip`/`.xlsx`) клікаються як
> завантаження — те саме гейтування, що в workspace-панелі.

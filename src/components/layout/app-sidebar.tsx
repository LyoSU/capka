        ...
        {!loaded && (
          <div className="space-y-1 px-2 py-1" aria-hidden>
            {[88, 72, 80, 64, 84, 70, 58].map((w, i) => (
              <div
                key={i}
                className="h-9 animate-pulse-fast rounded-md bg-sidebar-accent"
                style={{ width: `${w}%` }}
              />
            ))}
          </div>
      ...
      <SidebarFooter className="p-2">
        <DropdownMenu>
          <DropdownMenuTrigger
            ...
            className={cn(
              "flex w-full items-center gap-2 rounded-md px-3 py-1.5 text-left outline-none transition-colors",

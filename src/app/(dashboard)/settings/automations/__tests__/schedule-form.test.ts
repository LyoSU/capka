...
describe("automation schedule form — reading a stored trigger", () => {
  it("recognizes the three shapes it can write", () => {
    ...
    expect(toForm({ kind: "schedule", cron: "0 9 * * 1", timezone: TZ }, "UTC"))
      .toMatchObject({ freq: "weekly", time: "09:00", weekdays: ["1"] });
  ...
  it("reads a list of weekdays as one weekly schedule, deduplicated and in order", () => {
    expect(toForm({ kind: "schedule", cron: "0 9 * * 5,1,3,1", timezone: TZ }, "UTC"))
      .toMatchObject({ freq: "weekly", weekdays: ["1", "3", "5"] });
  });
  ...
  it("keeps an expression it cannot represent instead of flattening it", () => {
    ...
    // a step, a range, a month field, a day February doesn't always have.
    for (const cron of ["*/15 * * * *", "0 9 * * 1-5", "0 9 * * 1,7", "0 9 1 3 *", "0 9 31 * *"]) {
...
describe("automation schedule form — writing it back", () => {
  it("round-trips every shape it claims to understand", () => {
    for (const cron of ["30 7 * * *", "0 9 * * 1", "0 9 * * 1,2,3,4,5", "0 9 15 * *"]) {

...
export const SETTINGS_DIRECTORY: SettingsEntry[] = [
  ...
  { href: "/settings/users", label: "settings.usersPage.title", page: "settings.nav.people", keywords: "користувачі users роль role admin адмін доступ approve схвалити видалити remove запросити invite", adminOnly: true },
  { href: "/settings/users?tab=signin", label: "settings.authentication.mode.title", page: "settings.nav.people", keywords: "реєстрація registration signup вхід login закрити", adminOnly: true },
  { href: "/settings/users?tab=signin", label: "settings.authentication.telegram.title", page: "settings.nav.people", keywords: "telegram телеграм oidc вхід login", adminOnly: true },

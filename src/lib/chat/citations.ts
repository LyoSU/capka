...
/** `[3]` or `[1, 2]` — up to three digits, comma groups allowed. Never matches
 *  footnote syntax (`[^1]`) or a markdown link label (those are link nodes). */
...
export function makeRemarkCitations(sources: NumberedSource[]) {
  ...
  return () => (tree: Root) => {
    visit(tree, "text", (node, index, parent) => {
      ...
      // Never rewrite the caption of an existing link.
      ...
      // Continue after what we inserted (none of it is a text node to revisit).

...
describe("member-near-budget", () => {
  it("fires past 80% of the effective monthly cap", () => {
    const t = computeAttention(base({ members: [member({ monthCap: 100, sharedSpendMonth: 85 })] }));
  ...
  it("does not fire at exactly 80% or below", () => {
    const t = computeAttention(base({ members: [member({ monthCap: 100, sharedSpendMonth: 80 })] }));
    expect(t.find((x) => x.type === "member-near-budget")).toBeUndefined();
  });
  it("ignores unlimited (null cap) members", () => {
    const t = computeAttention(base({ members: [member({ monthCap: null, sharedSpendMonth: 9999 })] }));
    expect(t.find((x) => x.type === "member-near-budget")).toBeUndefined();
  });
  it("never fires in the own-key view", () => {
    const t = computeAttention(base({ scope: "own", members: [member({ monthCap: 100, sharedSpendMonth: 85 })] }));
    expect(t.find((x) => x.type === "member-near-budget")).toBeUndefined();
  });
});

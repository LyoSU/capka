# Deploying Capka on Fly.io (platform-only)
...
fly launch --no-deploy --copy-config --name capka   # edit name as needed

# 3. Provision Postgres and attach it — this sets DATABASE_URL automatically.
fly postgres create --name capka-db
fly postgres attach capka-db
...
fly secrets set \
  CAPKA_MASTER_KEY=$(openssl rand -hex 32) \
  CONTROLLER_SECRET=$(openssl rand -hex 32) \
  PUBLIC_URL=https://capka.fly.dev      # use your app's actual *.fly.dev host
...
- `deploy/fly.toml` pulls `ghcr.io/lyosu/capka-platform:latest` — publish that image

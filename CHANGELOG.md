...
- Streamed replies appear word by word again instead of in clumps of two or three: at an ordinary model's pace the client pacer was releasing a whole server batch per tick, so the cadence on screen was the 100 ms publish interval.
...
- Automations: optional "only run when…" condition (`run_when`, one sentence), checked by a small model call before each firing; an unmet condition skips the run visibly with its reason (`last_skip`), never as a failure. "Run now" ignores it.
...
- Chat, desktop: opening a file shows it inside the files panel beside the conversation instead of a dialog over it; Escape returns to the file list, closing the panel puts the file away. Phones keep the viewer, always full-screen and without the size toggle.
...
### Fixed
- Chat secrets: a sandbox command is refused (and can be retried) when the workspace's secret list cannot be loaded, instead of running with only this chat's values redacted. Line-wrapped base64 output (76 and 64 columns) is redacted too.
- Folder sync: `POST /api/folders/upload` refuses to write into a folder another window is syncing, whether or not the request names a lease, and answers only after every file write has finished.
- Chat: messages queued behind a running reply are held, not sent, while the chat's model is unavailable.
...
- Files: "open in a new tab" shows text and Markdown files (served as `text/plain`) instead of downloading them; the link is offered only for what a browser will display.
...
- Model picker: the brand rail names each company next to its glyph instead of showing glyphs alone.
